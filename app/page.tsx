...
export default function HomePage() {
  ...
  const getValidApiUrl = () => {
    ...
    // Si pas de variable d'environnement, utiliser l'URL par défaut
    ...
    try {
      ...
      console.warn(`Invalid API URL in environment variable: "${envUrl}". Using default.`)
    }
  ...
  const shouldUseApi = (url: string) => {
    // Ne pas essayer l'API si l'URL pointe vers des services d'hébergement frontend
    const frontendHosts = ["vercel.app", "netlify.app", "github.io", "surge.sh", "firebase.app", "web.app"]

    try {
      const parsedUrl = new URL(url)
      return !frontendHosts.some((host) => parsedUrl.hostname.includes(host))
    } catch {
      return false
    }
  }
  ...
  const loadData = async () => {
    ...
    // Si l'URL d'environnement n'est pas valide, utiliser directement le mode démo
    if (envUrl && envUrl !== apiUrl) {
      console.log("Invalid environment URL detected, using demo mode")
      setApiStatus({
        available: false,
        error: `URL d'environnement invalide: "${envUrl}"`,
        url: envUrl,
        mode: "error",
      })
    ...
    // Si l'URL pointe vers un service d'hébergement frontend, utiliser le mode démo
    if (!shouldUseApi(apiUrl)) {
      console.log("Frontend hosting URL detected, using demo mode")
      setApiStatus({
        available: false,
        error: "L'URL pointe vers un service d'hébergement frontend",
        url: apiUrl,
        mode: "demo",
      })
      setFeaturedHotels(fallbackHotels)
    ...
    try {
      ...
      const testResponse = await fetch(`${apiUrl}/hotels/featured`, {
        method: "GET",
        headers: {
      ...
      const hotelsData = await testResponse.json()

      // Test des locations
      const locationsResponse = await fetch(`${apiUrl}/hotels/locations`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const handleSearch = async () => {
    try {
      const searchParams = {
        ...
        guests: Number.parseInt(searchData.guests),
      }
    ...
    <AuthProvider>
      <NotificationProvider>
        <div className="min-h-screen bg-gray-50">
          ...
          {/* Messages de statut */}
          {apiStatus.mode === "error" && (
            <div className="bg-red-50 border-l-4 border-red-400 p-4">
              <div className="flex">
                <Info className="h-5 w-5 text-red-400 mr-3 mt-0.5" />
                <div className="ml-3">
                  <p className="text-sm text-red-700">
                    <strong>Configuration incorrecte :</strong> {apiStatus.error}
                  </p>
                  <p className="text-xs text-red-600 mt-1">
                    L'application fonctionne en mode démo avec des données d'exemple.
                  </p>
                  <details className="mt-2">
                    ...
                    <div className="mt-1 text-xs text-red-600">
                      <p>Mettez à jour la variable d'environnement NEXT_PUBLIC_API_URL avec :</p>
                      <ul className="list-disc list-inside ml-2 mt-1">
                        <li>Une URL valide (ex: http://localhost:8080/api)</li>
                        <li>Ou supprimez la variable pour utiliser l'URL par défaut</li>
                      </ul>
                    </div>
                  </details>
                </div>
          ...
          {apiStatus.mode === "demo" && !apiStatus.error?.includes("invalide") && (
                ...
                <div className="ml-3">
                  ...
                  <details className="mt-2">
                    ...
                    <div className="mt-1 text-xs text-blue-600">
                      ...
                      <ul className="list-disc list-inside ml-2 mt-1">
                        <li>Démarrez le backend Spring Boot sur localhost:8080</li>
                        <li>Ou mettez à jour NEXT_PUBLIC_API_URL avec l'URL de votre API</li>
                      </ul>
                    </div>
                  </details>
                </div>
